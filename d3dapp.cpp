...
CD3DApplication::CD3DApplication()
{
	...
	m_pd3dDeviceUtility = new CDirect3DDeviceUtility();
}
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
    ...
    if( SUCCEEDED(hr) )
    {
		...
		m_pd3dDeviceUtility->m_pd3dDevice = m_pd3dDevice;
		D3DVIEWPORT8 mp;
		mp.Height=m_d3dsdBackBuffer.Height;
		mp.Width=m_d3dsdBackBuffer.Width;
		mp.MaxZ=1.0;
		mp.MinZ=0;
		mp.X=0;
		mp.Y=0;
		m_pd3dDeviceUtility->SetViewportEx(&mp);
...
HRESULT CD3DApplication::UserSelectNewDevice()
{
    ...
    //if( long i=m_pd3dDevice->Release() > 0L )
    //    return DisplayErrorMsg( D3DAPPERR_NONZEROREFCOUNT, MSGERR_APPMUSTEXIT );
