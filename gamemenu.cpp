...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CMenuItem::Render( LPDIRECT3DDEVICE8 pd3dDevice, CDirect3DDeviceUtility *pd3dDeviceUtility, CD3DFont* pFont )
{
	...
	pd3dDeviceUtility->GetTransform( D3DTS_VIEW,       &matViewSaved );
	pd3dDeviceUtility->GetTransform( D3DTS_PROJECTION, &matProjSaved );
	...
	pd3dDeviceUtility->SetTransform( D3DTS_VIEW,       &matView );
	pd3dDeviceUtility->SetTransform( D3DTS_PROJECTION, &matProj );
	...
	pd3dDeviceUtility->SetRenderState( D3DRS_LIGHTING, TRUE );
	pd3dDeviceUtility->SetRenderState( D3DRS_AMBIENT,  0xffffffff );

	// Translate the menuitem into place
	D3DXMATRIX matWorld;
	D3DXMatrixScaling( &matWorld, 0.4f, 0.4f, 0.4f );
	matWorld._42 = (dwNumChildren*1.0f) + 2.0f;
	pd3dDeviceUtility->SetTransform( D3DTS_WORLD, &matWorld );
	...
	// Restore matrices
	pd3dDeviceUtility->SetTransform( D3DTS_VIEW,       &matViewSaved );
	pd3dDeviceUtility->SetTransform( D3DTS_PROJECTION, &matProjSaved );
